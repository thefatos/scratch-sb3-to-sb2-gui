...
exit_button = Button(root, text="Open File", command=root.destroy)
exit_button.pack(pady=20)

exit_button = Button(root, text="Kill the app.", command=sys.exit)
exit_button.pack(pady=40)

top=tk.Tk()
 
def helloCallBack():
    os.system('sb3tosb2.py -c')

    root.title('NORMAL MODE')
 
B=tk.Button(top,text="Compatabily Mode",command= helloCallBack)
B.pack()
top.mainloop()
...
def printWarning(message):
    ctypes.windll.user32.MessageBoxW(0, "WARNING", "WARNING", 1)


def printError(message, gui):
    ctypes.windll.user32.MessageBoxW(0, "Error has happened.", "READ ME", 1)
    exit()
